...
out_path = r'c:\Temp\familyhub\analysis_output6.txt'
...
def _has_lambda_before(target_match_start, stripped):
    arrow_pos = stripped.find('=>')
    return arrow_pos != -1 and arrow_pos < target_match_start
    ...
    for i, line in enumerate(lines):
        ...
        if awaits:
            if 'mounted' in stripped or 'context.mounted' in stripped:
                ...
                m = re.search(r'\bsetState\s*\(', stripped)
                if m and not stripped.startswith('//'):
                    if 'await ' not in stripped and not _has_lambda_before(m.start(), stripped):
                        add('setState after await without mounted check', path, i+1, stripped, 'HIGH')
                    awaits.clear()
                else:
                    m = re.search(r'Navigator\.(pop|push)', stripped)
                    if m and 'await ' not in stripped:
                        if not _has_lambda_before(m.start(), stripped) and 'onPressed:' not in stripped and 'onSubmitted:' not in stripped and 'onTap:' not in stripped and 'builder:' not in stripped and 'actions:' not in stripped and 'onChanged:' not in stripped:
                            add('Navigator after await without mounted check', path, i+1, stripped, 'HIGH')
                        awaits.clear()
                    else:
                        m = re.search(r'context\.push\(', stripped)
                        if m and 'await ' not in stripped:
                            if not _has_lambda_before(m.start(), stripped):
