...
class ChildDevelopmentRepository {
  static final ChildDevelopmentRepository _instance = ChildDevelopmentRepository._internal();
  ...
  Future<List<ChildDevelopmentLog>> getMyLogs({
    ...
    try {
      ...
      final response = await query
          ...
          .limit(limit);
      final list = (response as List).map((e) => ChildDevelopmentLog.fromJson(e)).toList();
  ...
  Future<List<ChildDevelopmentLog>> getLogsByType(DevelopmentLogType type) async {
  ...
  Stream<List<ChildDevelopmentLog>> watchMyLogs() {
    try {
      ...
      return _client
          ...
          .map((data) {
            final filtered = data.where((e) => e['child_id'] == childId).toList();
            filtered.sort((a, b) => (a['logged_at'] ?? '').toString().compareTo((b['logged_at'] ?? '').toString()));
            return filtered.map((e) => ChildDevelopmentLog.fromJson(e)).toList();
          });
    } catch (e, st) {
  ...
  Future<ChildDevelopmentLog> createLog({
    ...
    try {
      _checkSession();
      final response = await _client.from('child_development_logs').insert({
        'family_id': _familyId!,
        'child_id': _childId!,
        'log_type': logType.name,
        'value': value,
        'unit': unit,
        'logged_at': (loggedAt ?? DateTime.now()).toIso8601String().substring(0, 10),
        'notes': notes,
      }).select().single();
