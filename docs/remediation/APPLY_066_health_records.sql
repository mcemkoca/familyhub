...
-- ============================================
-- DOĞRULAMA (migration'dan sonra çalıştır)
-- Beklenen: tablo var, RLS aktif, 4 policy listelenir.
-- ============================================
select
  (select count(*) from information_schema.tables
     where table_schema='public' and table_name='health_records') as tablo_var,
  (select relrowsecurity from pg_class
     where relname='health_records' and relnamespace='public'::regnamespace) as rls_aktif,
  (select count(*) from pg_policies
     where schemaname='public' and tablename='health_records') as policy_sayisi;

-- Policy isimleri (4 tane beklenir: select/insert/update/delete)
